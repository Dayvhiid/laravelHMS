...
$data2 = [
    'patient_code' => $request->input('patient_code'),
    'srosphere' => $request->input('srossph'),
    ...
    'fosnva' => $request->input('fosnva'),
        ...
        // Use updateOrCreate method to either create or update the record
        $vital = Vital::updateOrCreate(
            ['patient_code' => $data['patient_code']], // Find record by patient_code
            $data // Data to be updated or created
        );
        $vital2 = vital2::updateOrCreate(
            ['patient_code' => $data['patient_code']],
           $data2
        );

        // Optionally, you can return a response
        // return response()->json(['message' => 'Vitals information stored successfully', 'data' => $vital], 201);
        return redirect(route('doctors.status'))->with('msg','Patients Vitals Stored succesfully'); 
    }
